...
| B-12 | `types/kanban.ts`, `App.tsx ROOM_MAP`, `RoutineAdherence.tsx` | 활동을 **이름 문자열**로 판별(`'운동'`, `'알고리즘'`, `slot.label === r.name`). 이름 바꾸면 세부 폼·방 이미지·이행률이 조용히 끊김 | 취약  ✅ 4단계 (활동 id 참조; 방 이미지·세부 폼 이름 매핑은 C11/A5 결정 대기) |
...
| 5. UI 다듬기 | 🔶 시안 B(종이 스튜디오) 선택 → 토큰·카드·칩·헤더 반영, 섹션 제목 위계 통일. 남은 것: 빈 상태·로딩(C16), 문체(C14), 활동색 대비(C13), 모바일 여백, 저장 토스트 | 6cc84aa, (이번 커밋) |
| 6~8 | 미착수 | |
