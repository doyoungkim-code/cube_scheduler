...
- 활동색 위의 글자색은 `src/lib/color.ts` 의 `inkOn` 이 대비를 계산해 정한다 (밝은 색이면 짙은 잉크, 어두우면 흰색).
- 문구는 해요체로 통일했다. 칸반 티켓의 영문 스탬프(TO DO / IN PROGRESS / COMPLETE)는 기차표 인쇄 느낌을 위한 장식이라 그대로 둔다.
